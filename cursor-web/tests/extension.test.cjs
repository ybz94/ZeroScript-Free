...
function content(options = {}) {
  ...
  const provider = {
    ...
    version:options.providerVersion === undefined ? '0.4.22' : options.providerVersion, // null => pre-0.4.9 (no version field)
...
test('session announcement reports provider version and sync state',()=>{
  ...
  assert.equal(s.transportVersion,'0.4.22');
  assert.equal(s.providerVersion,'0.4.22');
...
test('model protocol returns code block extraction rather than rendered reply',async()=>{
  ...
  assert.equal(r.diagnostics.version,'0.4.22');
});
