...
### Added
- **External MCP clients can now connect to the Bridge over HTTP (Streamable HTTP).** The Bridge exposes the exact same tool set - Roblox Studio, every added server, the virtual commands - on a second, token-protected endpoint: `http://127.0.0.1:17614/mcp/<token>` (port configurable via `ZS_MCP_HTTP_PORT`, `0` disables). The per-launch random token is printed to the console and written to `mcp_http_url.txt` next to the bridge; wrong tokens get a bare 404 so the endpoint is undiscoverable. With a tunnel (e.g. `cloudflared tunnel --url http://127.0.0.1:17614`) the public URL works in any HTTP MCP client - Arena.ai Agent Mode, Claude Desktop, Cursor, ... - driving the same local servers the extension drives. The endpoint speaks the 2025-06-18 Streamable HTTP protocol: `initialize` with `Mcp-Session-Id` session handling, `tools/list`, `tools/call` (failures arrive as proper `isError` results, not transport errors), `notifications/*` and `DELETE` for session teardown. The URL is a credential: it grants full read/write access to the local MCP servers, including Luau execution in Studio.
- **Replies now follow the user's language.** The extension passes the browser language into the system prompt: a Chinese browser gets an explicit "reply in 简体中文" directive, every other language gets "answer in the language the user writes in". Commands, JSON, tool names, code and paths are still never translated - only the model's prose. (Before this, the all-English prompt + English tool results pulled the model into English even for Chinese users.)
- **Generic MCP mode: any local MCP server works, Roblox Studio is no longer required.** The bridge, the system prompt, and the status UI are now topology-aware. With a `roblox` server configured (the default) behaviour is exactly as before; with any other server set - filesystem, git, GitHub, Blender, ... - the agent discovers servers via `list_mcp_servers`, lists per-server commands via `list_commands` with a `"server"` param, and the status dot goes green when any connected server has tools. The Roblox-specific prompt sections (place discovery, `execute_luau` format, project memory) and the place-loaded status are automatically skipped when no roblox server is configured.
- **MCP servers menu: every server is first-class.** The ⋯ menu's MCP servers section now lists ALL configured servers (Roblox included) with per-server health; any of them can be removed, including Roblox Studio, for a fully generic setup.
- **Bridge: clean boot on generic server sets.** All Studio-specific machinery (orphan StudioMCP cleanup, Studio MCP port forensics, the studio recovery watcher, the Studio process scan) only runs when a `roblox` server is configured; with none, the bridge reports a definite "not applicable" instead of "unknown", and an empty config boots with a hint on how to add a server.
